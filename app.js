/* FriOlKa PWA – v9 (Tile→Empfehlungen, Event-Delegation, Scroll-into-view, Toast) */
...
function showToast(msg){ const t=$('#toast'); if(!t) return; t.textContent=msg; t.style.display='block'; clearTimeout(showToast._tmr); showToast._tmr=setTimeout(()=>{t.style.display='none'}, 1800); }
...
// Event delegation for tiles (click + touchend)
function onTileActivate(tab){ if(!tab) return; activateTab(tab); if (tab==='nearby'){ state.viewMode='recommendations'; ensureLocationAndData().then(()=>{ renderNearby(); const el=$('#tab-nearby'); if(el) el.scrollIntoView({behavior:'smooth', block:'start'}); }); } else if (tab==='map'){ activateTab('map'); initMapOnce(); setTimeout(()=>{ const el=$('#tab-map'); if(el) el.scrollIntoView({behavior:'smooth', block:'start'}); renderMap(); },0); } else { const el=$('#tab-'+tab); if(el) el.scrollIntoView({behavior:'smooth', block:'start'}); }
}
document.addEventListener('click', (e)=>{ const a=e.target.closest('.tile[data-go]'); if(a){ e.preventDefault(); onTileActivate(a.getAttribute('data-go')); }});
document.addEventListener('touchend', (e)=>{ const a=e.target.closest('.tile[data-go]'); if(a){ e.preventDefault(); onTileActivate(a.getAttribute('data-go')); }});
...
async function ensureLocationAndData(){ if (!state.pos){ await getLocation(false); } if (state.pos && (!state.lastFetchPos || state.spots.length===0)){ showToast('Lade Empfehlungen …'); await refreshAll(); state.lastFetchPos = {lat:state.pos.lat, lon:state.pos.lon}; } }
...
async function getLocation(watch=false){ if (!navigator.geolocation){ showError('Geolocation wird nicht unterstützt.'); setGeoState('Nicht unterstützt','err'); return; } try{ setGeoState('Ortung…','wait'); refreshBtn && (refreshBtn.disabled=true); await new Promise((resolve,reject)=>{ navigator.geolocation.getCurrentPosition(async (pos)=>{ const p = { lat: pos.coords.latitude, lon: pos.coords.longitude, acc: pos.coords.accuracy }; const movedKm = state.pos ? distKm(state.pos.lat,state.pos.lon,p.lat,p.lon) : 999; state.pos = p; setGeoState('OK'); startAutoRefresh(); await ensureMap(); if (!state.lastFetchPos || movedKm>2){ await refreshAll(); state.lastFetchPos = p; } renderAll(); resolve(); if (watch){ navigator.geolocation.watchPosition(async (pp)=>{ const np = { lat: pp.coords.latitude, lon: pp.coords.longitude, acc: pp.coords.accuracy }; const mk = distKm(state.pos.lat,state.pos.lon,np.lat,np.lon); state.pos = np; setGeoState('OK'); await ensureMap(); if (mk>2){ await refreshAll(); state.lastFetchPos=np; } renderAll(); }, (err)=>{ showError(err.message); setGeoState('Fehler','err'); }, { enableHighAccuracy:true, maximumAge:10000, timeout:20000 }); } }, (err)=>{ showError('Standort nicht verfügbar: '+err.message); setGeoState('Fehler','err'); reject(err); }, { enableHighAccuracy:true, maximumAge:0, timeout:20000 }); }); } finally { refreshBtn && (refreshBtn.disabled=false); } }
...
function buildRecommendationsHtml(){ const within=(km,cats=[])=>state.spots.filter(s=>distKm(state.pos.lat,state.pos.lon,s.lat,s.lon)<=km && (cats.length?cats.includes(s.category):true));
  ...
  const sec = (title, items)=>{ if(!items.length) return ''; const rows = topN(items,5).map(s=>{ const d=distKm(state.pos.lat,state.pos.lon,s.lat,s.lon).toFixed(1)+' km'; const badges = badgeTextFor(s).concat(qualityBadgesPlain(s)); return `<div class="item"><div class="lhs"><div class="name">${escapeHtml(s.name)}</div><div class="meta">${d} · ${escapeHtml(s.category)}</div><div class="badge-line">${badges.map(b=>`<span class=badge>${escapeHtml(b)}</span>`).join(' ')}</div></div><div class="actions"><button class="small-btn" onclick="Friolika.showDetail('${s.id}')">Details</button><a class="small-btn" href="https://www.google.com/maps/dir/?api=1&destination=${s.lat},${s.lon}&travelmode=driving" target="_blank" rel="noopener">Route</a></div></div>`; }).join(''); return `<div class="section"><h3>${title}</h3>${rows}</div>`; };
...
// Start
document.addEventListener('DOMContentLoaded', async ()=>{ renderSpotList(); try{ if (navigator.permissions && navigator.permissions.query){ const p = await navigator.permissions.query({name:'geolocation'}); if (p.state==='granted'){ await ensureLocationAndData(); renderAll(); } } }catch{} });
