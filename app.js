...
/* FriOlKa V13 – schneller & robuster (Kachel-Fix, Caching, Speed-Mode) */
...
let state={pos:null,lastFetchKey:null,weather:null,spots:[],imported:[],quality:{},basecamps:[],viewMode:'recommendations',speed:false};
...
const cacheStore={load(){try{return JSON.parse(localStorage.getItem('friolika-cache')||'{}')}catch{return{}}},save(o){localStorage.setItem('friolika-cache',JSON.stringify(o))}};
...
// Kachel-Klicks (click & touchend) – robust, verhindert Default-Scroll zu #hash
...
function onTile(tab){activateTab(tab); if(tab==='nearby'){ state.viewMode='recommendations'; ensureLocation().then(()=>{ renderNearby(); scrollToTab('nearby'); ensureDataAndMaybeFetch(); }); } else if(tab==='map'){ if(!state.speed) initMapOnce(); setTimeout(()=>{ renderMap(); scrollToTab('map'); },0); } else { scrollToTab(tab); }}
function scrollToTab(t){ document.getElementById('tab-'+t)?.scrollIntoView({behavior:'smooth',block:'start'}); }
...
$('#cb-speed')?.addEventListener('change',e=>{ state.speed=e.target.checked; storage.save(); if(state.speed){ showToast('Speed‑Modus aktiv (Karte & rechenintensive Aufgaben verzögert)'); }})
...
// --------- Data Pipeline (Fast First, then Fresh) ---------
let inflight=null; const endpoints=['https://overpass-api.de/api/interpreter','https://overpass.kumi.systems/api/interpreter','https://overpass.openstreetmap.ru/api/interpreter'];
...
function ensureDataAndMaybeFetch(force=false){ if(!state.pos) return; const key=cellKey(state.pos.lat,state.pos.lon); const cache=cacheStore.load(); const entry=cache[key]; const fresh=entry && (Date.now()-entry.ts < CACHE_TTL_H*3600*1000);
 ...
 if(entry){ // fast path
   state.spots=mergeImported(entry.spots||[]);
   renderAll(); logDiag('INFO',`Cache-Hit ${key} (${state.spots.length} Spots)`);
 }
 if(force || !fresh){ // refresh in background
   fetchSpotsFromOSM(key).then(spots=>{ if(spots&&spots.length){ cache[key]={ts:Date.now(),spots}; cacheStore.save(cache); state.spots=mergeImported(spots); renderAll(); logDiag('INFO',`Cache-Update ${key}: ${spots.length} Spots`);} else { logDiag('WARN','Overpass lieferte 0 Elemente'); }}).catch(e=>{ logDiag('ERROR','Fetch fehlgeschlagen: '+e); });
 } else {
   logDiag('INFO',`Cache frisch (<=${CACHE_TTL_H}h), kein Fetch`);
 }
}
...
async function fetchSpotsFromOSM(key){ if(inflight){ logDiag('INFO','Fetch bereits läuft'); return []; }
 inflight=true; try{
   ...
   // rotate endpoints with timeout/abort
...
function renderNearby(){ const reco=$('#nearby-reco'), list=$('#nearby-list'); if(!reco||!list) return; if(!state.pos){ reco.innerHTML='<div class="msg">Tippe 🔄 oder „Ort wählen“.</div>'; list.style.display='none'; return; }
  if(state.viewMode==='recommendations'){
    ...
    const spots=state.spots||[]; if(spots.length===0){ reco.innerHTML='<div class="msg">Noch keine Daten. Ich aktualisiere im Hintergrund …</div>'; return; }
    const within=(km,cat)=>spots.filter(s=>distKm(state.pos.lat,state.pos.lon,s.lat,s.lon)<=km && (!cat||cat.includes(s.category)));
    ...
    const parts=[
      ...
      sec('🏛️ Kultur & Sehenswürdigkeiten', within(25,['culture']).sort(dsort)),
    ...
    reco.innerHTML = parts || '<div class="msg">Im Cache keine passenden Kategorien – warte auf Aktualisierung oder nutze die Chips oben.</div>';
  } else {
...
function renderSpotList(){ const ul=$('#spot-list'); if(!ul) return; ul.innerHTML=''; (state.imported||[]).forEach(s=>{ const li=document.createElement('li'); li.className='card'; li.innerHTML=`<b>${escapeHtml(s.name)}</b> <span class='badge'>${s.category}</span><div class='muted'>${escapeHtml(s.details||'')}</div>`; ul.appendChild(li); }); }

// --------- Map (lazy if speed mode) ---------
...
function renderMap(){ if(state.speed) return; if(!map){ initMapOnce(); if(!map) return; }
  markersLayer.clearLayers(); (state.spots||[]).forEach(s=>{ const m=L.marker([s.lat,s.lon]).addTo(markersLayer); m.bindPopup(`<div class='card'><b>${escapeHtml(s.name)}</b><br/>${escapeHtml(s.details||'')}<div style='margin-top:6px'><a class='chip' href='https://www.google.com/maps/dir/?api=1&destination=${s.lat},${s.lon}' target='_blank' rel='noopener'>Route</a></div></div>`); });
  if(state.pos){ if(!userMarker) userMarker=L.marker([state.pos.lat,state.pos.lon],{opacity:0.6}).addTo(map); else userMarker.setLatLng([state.pos.lat,state.pos.lon]); map.setView([state.pos.lat,state.pos.lon], 11, {animate:true}); }
}
...
document.addEventListener('DOMContentLoaded', async()=>{
  renderQuickSearch();
  // bei Start NICHT automatisch Map laden (Speed)
  try{ if(navigator.permissions && navigator.permissions.query){ const p=await navigator.permissions.query({name:'geolocation'}); if(p.state==='granted'){ await ensureLocation(); ensureDataAndMaybeFetch(); renderAll(); } else { renderNearby(); } } }catch{ renderNearby(); }
});
